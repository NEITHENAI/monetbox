...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"paintings" | "events" | "dashboard" | "applications" | "artists">("dashboard");
  ...
  const [form, setForm] = useState({
    title: "", artist: "", price: "", imagePreview: "", description: "",
  ...
  const resetForm = () => {
    setForm({ title: "", artist: "", price: "", imagePreview: "", description: "", dimensions: "", medium: "", year: "", category: "Abstract" });
    setEditingId(null);
    setShowForm(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        const { imagePreview, ...formData } = form;
        const success = await updatePainting(editingId, { ...formData, imageUrls: [finalImageUrl], price: Number(form.price), year: Number(form.year) });
        if (success) {
  ...
  const handleEdit = (p: Painting) => {
    setForm({
      title: p.title, artist: p.artist, price: String(p.price), imagePreview: p.imageUrls?.[0] || "",
    ...
    <div className={styles.container}>
      ...
      <aside className={styles.sidebar}>
        ...
        <nav className={styles.sidebarNav}>
          ...
          <a href="#" className={activeTab === "applications" ? styles.navItemActive : styles.navItem} onClick={() => setActiveTab("applications")}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
            Applications
          </a>
          <a href="#" className={activeTab === "artists" ? styles.navItemActive : styles.navItem} onClick={() => setActiveTab("artists")}>
      ...
      <div className={styles.main}>
        {activeTab === "events" ? (
          ...
          <ApplicationsPanel showToast={showToast} />
        ) : activeTab === "artists" ? (
          <ArtistsPanel showToast={showToast} />
        ) : (
          <>
            {/* Header */}
                  ...
                  <tr key={painting.id}>
                    <td>
                      <div className={styles.paintingCell}>
                        <img src={painting.imageUrls?.[0]} alt={painting.title} className={styles.tableThumb} />
